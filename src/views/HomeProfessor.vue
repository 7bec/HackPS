    ...
    div.content
      v-ons-row(width='100%' vertical-align='top')
        v-ons-col(style='color: #172B3E')
          div.font1 Olá, <b>Breno</b>
          div.font2 Acompanhe seus alunos
            ...
            v-card.cardServices(dark style='border-radius: 10px;')
              v-ons-ripple
                div(style='height: 100%; width: 100%; padding: 8px 8px 8px 8px;')
                  ...
                  div.font5(style='line-height: 16px') Chats
          v-col(cols='4' style='padding: 4px')
            v-card.cardServices(dark style='border-radius: 10px;')
              v-ons-ripple
                div(style='height: 100%; width: 100%; padding: 8px 8px 8px 8px;')
                  ...
                  div.font5(style='line-height: 16px') Encontre novos alunos
          v-col(cols='4' style='padding: 4px')
            v-card.cardServices(dark style='border-radius: 10px;')
              v-ons-ripple
                div(style='height: 100%; width: 100%; padding: 8px 8px 8px 8px;')
      ...
      v-ons-row(width='100%' style='padding: 16px 0 8px 0;')
        ...
        v-row(no-gutters style='max-width: calc(100% + 10px); position: relative;')
          v-slide-group.sliderGroupSetts(v-model='model' style='max-width: 100%;')
            v-slide-item(v-for='(aluno, i) in alunos' :key='i')
              v-card.borderInstrutor.boxDone(v-if='i == 0' outlined height='250' width='200' :class='{ "doneBadge": aluno.done }' style='margin: 4px 4px 20px 1px; overflow: hidden;')
                v-img(height='250' :src='aluno.img' style='border-bottom-left-radius: 0px; border-bottom-right-radius: 0px; position:relative;') 
                  v-badge( avatar  class='badge')
                    template(v-slot:badge)
                      v-avatar(color='rgba(240,84,41,1)') {{aluno.badge}}        
                v-container(style='padding-top: 5px; position: absolute; bottom: 0; width: 100%; height: 70px; background: rgba(48, 27, 20, 0.5);')
                  v-layout(column)
                    v-flex(xs12)
                      h5(style='display: inline; margin-bottom: 0; font-weight: 700; font-size: 14.5px; line-height: 14.5px; letter-spacing: -0.83px; color: white;') {{aluno.nome}}  
                  ...
                  v-row(no-gutters style='padding: 0;')
                    v-col()
                      h5(style='margin: 5px 0 0 0; font-weight: 700; font-size: 12px; letter-spacing: -0.83px; color: rgba(255,255,255,0.9); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;') 
                  v-row(no-gutters style='padding: 0;')
                    v-col(style='padding: 3px 0 0 0;')
                      .modalidadeBadge {{ aluno.modalidade }}
              v-card.borderInstrutor.boxDone(v-else outlined height='250' width='200' :class='{ "doneBadge": aluno.done }' style='margin: 4px 4px 20px 4px; overflow: hidden;')
                v-img(height='250' :src='aluno.picture_url' style='border-bottom-left-radius: 0px; border-bottom-right-radius: 0px')
                v-container(style='padding-top: 5px; position: absolute; bottom: 0; width: 100%; height: 70px; background: rgba(48, 27, 20, 0.5);')
                  v-layout(column)
                    v-flex(xs12)    
                      span(style='position: relative; top: 2px;')
...
export default {
  ...
  created () {
    const that = this
    this.$fiery(firebase.firestore().collection('alunos'), {
