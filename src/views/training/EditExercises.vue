  ...
  v-ons-page
    ...
    div.content
      ...
      v-ons-row(width='100%' style='padding: 0 0 22px 0')
        v-row(v-for='session in sessions' draggable style='padding: 0; width: 100%;')
          v-col(cols='6' style='padding: 0;')
            h3.list-custom-header {{ session }}
          ...
          v-row(no-gutters style='width: 100%; margin-bottom: 4px;')
            v-col(v-if='(exercise.session === session) && j < 4' v-for='(exercise, j) in exercises' :key='j' cols='6' style='padding: 2px 4px;')
...
export default {
  ...
  mounted () {
    for (let i = 0; i < this.exercises.length; i++) {
      let session = this.exercises[i].session
  ...
  methods: {
    selectItem () {
      for(let j = 0; this.exercises.length > j; j++) {
        if (this.exercises[j].isSelected) {
